...
The interview transcript is generated by Speech-to-Text (STT) and WILL contain transcription errors, phonetic mistakes, and homophones (e.g., "docker" $\rightarrow$ "darker", "redis" $\rightarrow$ "read us", "kubectl" $\rightarrow$ "cube control").
...
- Only penalize if the candidate's core architectural logic or conceptual reasoning is fundamentally flawed, not if a single word was mis-transcribed.
