-- 1. Run this in your Supabase SQL Editor to create the table
create table if not exists popular_interviews (
  ...
  skills text[],
  overview text,
  company_traits jsonb,
  rounds jsonb,
  icon_url text
);

-- 2. Enable RLS
alter table popular_interviews enable row level security;

-- 3. Allow everyone to read
create policy "Enable read access for all users"
on "public"."popular_interviews"
for SELECT
to public
using (true);

-- 4. INSERT DATA (Google, Amazon, Meta)
...
(
    ...
    'Intermediate',
    '60 min',
    ARRAY['Advanced DSA Reasoning', 'Algorithmic Trade-offs', 'Verbal Complexity Analysis', 'Googliness', 'Leadership without Authority'],
    'For this role Google emphasizes strong data structures and algorithms skills, the ability to reason clearly about problem-solving strategies, and to communicate trade-offs and complexity verbally during interviews. Behavioral signals around General Cognitive Ability and "Googliness" are also critical, with focus on ownership, collaboration, and navigating ambiguity in addition to technical depth.',
    ...
    $$
    {
        "techRoundOne": {
            ...
            "questions": [
                "Given an integer array, return the length of the longest contiguous subarray with sum equal to a target K; explain both a brute-force and an optimized approach using prefix sums and hashing.",
                "You are given a sorted array that has been rotated at an unknown pivot; find the minimum element in O(log n) time and justify your choice of invariants.",
                "Given an array of integers, move all zeros to the end while maintaining the relative order of non-zero elements; discuss in-place strategies and stability considerations.",
                "Given a binary array, find the maximum number of consecutive 1s if you can flip at most one 0; describe how you would track window boundaries efficiently.",
                "Given an array of positive integers and a target sum S, determine whether there exists a contiguous subarray whose sum is exactly S and explain how you handle large inputs without overflow.",
                "Given a string, return the length of the longest substring without repeating characters and walk through your sliding-window logic in detail.",
                "Given two strings s and t, determine if t is an anagram of s and then extend your solution to handle Unicode and streaming input.",
                "Given a string containing only '(', ')', '{', '}', '[' and ']', determine if the input string is valid; discuss how you would detect subtle edge cases like prefix violations.",
                "You are given a string s; compress it by replacing consecutive repeated characters with the character followed by the count (e.g., aaabb → a3b2) and discuss when in-place modification is feasible.",
                "Implement a function to detect whether a singly linked list has a cycle and, if so, return the node where the cycle starts; compare different approaches and their space trade-offs.",
                "Given a singly linked list and an integer k, reverse the nodes of the list in groups of k; explain how you would manage pointers and avoid extra space.",
                "Merge two sorted singly linked lists into one sorted list; then discuss how your approach changes if the lists are extremely long and must be processed as streams.",
...
(
    ...
    'Intermediate',
    '80 min',
    ARRAY['DSA Problem Solving', 'LLD Design', 'System Scalability', 'Leadership Principles', 'Customer Obsession'],
    'Amazon SDE II interviews typically feature an Online Assessment followed by a 4-round onsite loop with 3-4 coding/LLD sessions, one system design, and behavioral questions tied to Leadership Principles (LPs) in every round. Emphasis is on medium-hard LeetCode-style problems, object-oriented design, scalable systems, and demonstrating LPs like Ownership, Customer Obsession, Dive Deep, and Bias for Action through STAR responses.',
...
(
    ...
    'Intermediate',
    '80 min',
    ARRAY['LeetCode Medium-Hard Coding', 'System/Product Design', 'Behavioral Authenticity', 'Complexity Analysis', 'Scalability Reasoning'],
