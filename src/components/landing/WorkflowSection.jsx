...
const WorkflowSection = () => {
            ...
            steps.forEach((_, i) => {
                ...
                tl.to(`.step-card-${i} .step-text`, { y: -50, opacity: 0, duration: 1 })
                    ...
                    .fromTo(`.step-card-${i + 1}`,
                        { autoAlpha: 0 },
                        { autoAlpha: 1, duration: 0.1 } // Make container visible
                    )
    ...
    return (
        <section ref={sectionRef} className="relative bg-black overflow-hidden h-screen">
            {/* Background Glows */}
            ...
            <div ref={containerRef} className="w-full max-w-7xl mx-auto px-6 h-full relative z-10 flex flex-col justify-center">
                ...
                {steps.map((step, index) => (
                    <div
                        key={step.id}
                        className={`step-card step-card-${index} absolute top-1/2 -translate-y-1/2 left-0 w-full px-6 md:px-8 flex flex-col md:flex-row items-center gap-8 md:gap-20`}
                    >
                        {/* Text Content */}
                        <div className="step-text w-full md:w-5/12 flex flex-col gap-6 order-2 md:order-1">
                            {/* Step Indicator */}
                            <div className={`inline-flex items-center self-start gap-2 px-4 py-1.5 rounded-full bg-white bg-opacity-10 border border-white/10`}>
